...
classify_label2id = {'时政': 0, '房产': 1, '财经': 2, '科技': 3, '时尚': 4, '教育': 5, '家居': 6}
classify_id2label = {0: '时政', 1: '房产', 2: '财经', 3: '科技', 4: '时尚', 5: '教育', 6: '家居'}
...
def load_data(filename, task='category'):
    df = pd.read_csv(filename)
    if task == 'category':
        df['label'] = df['class_label'].apply(lambda x: classify_label2id[x])
    text = []
    label = []
    res = []
...
data = load_data('/ccf_multi_cls/labeled_data.csv')
train_data = data[:int(0.7*len(data))]
vaild_data = data[int(0.7*len(data)):]
